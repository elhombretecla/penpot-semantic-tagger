...
export function extractComprehensiveStyles(shape: any, _isInFlexContainer: boolean = false): Record<string, string> {
  ...
  try {
    ...
    if (shape?.type === 'text' || shape?.content) {
      ...
      // Text alignment
      if (shape?.textAlignHorizontal && typeof shape.textAlignHorizontal === 'string') {
        ...
        styles.textAlign = alignMap[shape.textAlignHorizontal] || shape.textAlignHorizontal.toLowerCase();
      }
      
      // Vertical text alignment (can influence flexbox alignment)
      if (shape?.textAlignVertical && typeof shape.textAlignVertical === 'string') {
        const verticalAlignMap: Record<string, string> = {
          'TOP': 'flex-start',
          'CENTER': 'center',
          'BOTTOM': 'flex-end'
        };
        // This could be used for align-items in a flex container
        const alignValue = verticalAlignMap[shape.textAlignVertical];
        if (alignValue) {
      ...
      if (shape?.textDecoration && typeof shape.textDecoration === 'string') {
        const decorationMap: Record<string, string> = {
          'UNDERLINE': 'underline',
          'STRIKETHROUGH': 'line-through',
          'NONE': 'none'
        };
        styles.textDecoration = decorationMap[shape.textDecoration] || shape.textDecoration.toLowerCase();
      }
      
      // Text transform (uppercase, lowercase)
      if (shape?.textTransform && typeof shape.textTransform === 'string') {
        ...
        styles.textTransform = transformMap[shape.textTransform] || shape.textTransform.toLowerCase();
      }
      ...
      // Letter spacing
      if (typeof shape?.letterSpacing === 'number') {
